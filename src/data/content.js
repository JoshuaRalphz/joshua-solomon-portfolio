...
export const experience = [
  {
    ...
    bullets: [
      ...
      'Architected and shipped AGC HUB — a custom internal PWA replacing a scattered tool stack (Google Drive, ClickUp, Trello, GHL, Canva) with one tailored platform for the agency. Unifies client resources and content calendars per-client + cross-client; powers the full production flow (task assignment → team submission → review via Canva + GHL APIs → PM scheduling). Migrated the legacy content calendar from Google Sheets via API. Stack: React + Vite + Firebase + GHL API + Canva API + Google Sheets API. In daily production across the agency team.',
